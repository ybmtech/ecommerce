...
if(isset($_POST['name'])){
    ...
    if(!$token || $token!==$_SESSION['_token']){
        ...
        if ($check_category !== false) {
            $msg = json_encode(['status' => false, 'message' => 'This category already exist.','error' => 'database']);
            echo $msg;
            exit();
        }
        else{
            ...
            if($result){
                $msg = json_encode(['status' => true, 'message' => 'Category Added']);
                  echo $msg;
                exit();
            }
            else{
                $msg = json_encode(['status' => false, 'message' => 'Opps, fail to add category,try again later ', 'error' => 'database']);
                echo $msg;
                exit();
            }
        }
